...
<span class="action-span1"><a href="{:U('Index/main')}?act=main">SHOP 管理中心</a> </span><span id="search_id" class="action-span1"> - 修改分类 </span>
...
<div class="main-div">
  <form action="" method="post" name="theForm">
	 <table width="100%" id="general-table">
		 <input type="hidden" name="cate_id" value="<?=$one_cate['cate_id']?>">
		<tbody>
			<tr>
				<td class="label">分类名称:</td>
				<td><input type="text" name="cate_name" maxlength="20" size="27" placeholder="字母数字下划线或汉字" value="<?=$one_cate['cate_name']?>"></td>
			</tr>
			<tr>
				...
				<td>
					<select name="cate_path" class="select">
						...
						<?php foreach($cate as $val):?>
							<?php if($one_cate['pid']==$val['cate_id']):?>
							<option value="<?=$val['full_path']?>" selected><?=str_repeat("&nbsp;&nbsp;&nbsp;",substr_count($val['full_path'],',')).$val['cate_name']?></option>
								<?php else:?>
			...
			<tr id="measure_unit">
				...
				<td><input type="text" name="cate_dan" size="12" placeholder="请输入单位" value="<?=$one_cate['cate_dan']?>"></td>
			</tr>
			<tr>
				<td class="label">排序:</td>
				<td><input type="text" name="cate_order" size="15" placeholder="可填可不填哦" value="<?=$one_cate['cate_order']?>"></td>
			</tr>

			<tr>
				<td class="label">是否显示:</td>
				<td>
					<?php if($one_cate['cate_show']==1):?>
					<input type="radio" name="cate_show" value="1" checked > 是<input type="radio" name="cate_show" value="0"> 否
					<?php else:?>
					<input type="radio" name="cate_show" value="1"> 是<input type="radio" name="cate_show" value="0" checked > 否
					<?php endif;?>
				</td>
			</tr>
			<tr>
				...
				<td>
					<?php if($one_cate['cate_nav']==1):?>
					<input type="radio" name="cate_nav" value="1" checked> 是  <input type="radio" name="cate_nav" value="0"> 否
					<?php else:?>
					<input type="radio" name="cate_nav" value="1"> 是  <input type="radio" name="cate_nav" value="0" checked> 否
					<?php endif;?>
				</td>
			</tr>
      <tr>
        ...
        <td>
          <textarea name="cate_content" rows="6" cols="48" id="content"><?=$one_cate['cate_content']?></textarea>
